...
export const combine = <T extends (Stream | Observable)[]>(...args$: T) => {
  ...
  if (!checkStreamOrObservableInput(args$, true)) {
    throw new Error(
      "combine operator only accepts Stream or Observable as input"
    );
  }

  const stream$ = (getGlobalFluthFactory()?.(
    args$.map((arg$) =>
      arg$._getProtectedProperty("_v")
    ) as StreamTupleValues<T>
  ) ||
  ...
  const promiseStatus = [...Array(args$.length)].map(
