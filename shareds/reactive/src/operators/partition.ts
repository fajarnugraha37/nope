...
export const partition = <T>(
  ...
  if (!checkStreamOrObservableInput(stream$)) {
    throw new Error(
      "partition operator only accepts Stream or Observable as input"
    );
  }

  const selectedStream$ = (getGlobalFluthFactory()?.() ||
    new Stream<T>()) as Stream<T>;
  const unselectedStream$ = (getGlobalFluthFactory()?.() ||
    new Stream<T>()) as Stream<T>;
