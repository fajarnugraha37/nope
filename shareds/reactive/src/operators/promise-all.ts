...
import type { StreamTupleValues } from "../reactive.js";
import {
...
const promiseAllImpl = <T extends (Stream | Observable)[]>(
  ...
  if (!checkStreamOrObservableInput(args$, true)) {
    throw new Error(
      "promiseAll operator only accepts Stream or Observable as input"
    );
  }

  const stream$ = (getGlobalFluthFactory()?.() ||
    new Stream<StreamTupleValues<T>>()) as Stream<StreamTupleValues<T>>;
  const payload: StreamTupleValues<T> = [] as any;
  const promiseStatus: PromiseStatus[] = [...Array(args$.length)].map(
  ...
  args$.forEach((arg$, index) => {
    if (arg$._getProtectedProperty("_finishFlag")) {
      finishCount += 1;
      payload[index] = arg$._getProtectedProperty(
        "_v"
      ) as StreamTupleValues<T>[number];
