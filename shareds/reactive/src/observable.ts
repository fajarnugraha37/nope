...
export class Observable<T = any> {
  ...
  use<P extends PluginParams<T>[]>(...plugins: P) {
    ...
    if (
      this.#parent &&
      (curPlugin.thenAll.length || curPlugin.executeAll.length)
    ) {
      throw new Error(
        "observable node can not use thenAll or executeAll plugin"
      );
    }

    const pluginKeys = Object.keys(
      curPlugin
    ) as (keyof Plugin<TextEncoderStream>)[];
  ...
  #unsubscribeObservable(active = false) {
    ...
    if (active) {
      this.#finishCallbackList.forEach((fn) =>
        safeCallback(fn)(this._v, this.status)
      );
      this._finishFlag = true;
    }
    ...
    if (active) {
      const childPending = this.#children.some(
        (child) => child.status === PromiseStatus.PENDING
      );
  ...
  offUnsubscribe(callback: () => void) {
    this.#unsubscribeCallbackList = this.#unsubscribeCallbackList.filter(
      (fn) => fn !== callback
    );
  }
  ...
  afterSetValue(callback: (value: T) => void) {
    if (
      !this.#afterSetValueCallbackList.includes(callback) &&
      !this._finishFlag
    )
  ...
  offComplete(callback: (value: T, status: PromiseStatus) => void) {
    this.#finishCallbackList = this.#finishCallbackList.filter(
      (fn) => fn !== callback
    );
  }
  ...
  #thenObserver<F>({
    ...
    const observer = new Observable<F extends PromiseLike<infer V> ? V : F>(
      this
    );
  ...
  thenImmediateSet(
    ...
    if (!this.#parent)
      this.status = this.status === null ? PromiseStatus.RESOLVED : this.status;
    return this.#thenObserver<T>({
      immediate: true,
      onfulfilled: (value: T) => this.#set(value, setter),
    });
  ...
  #runExecutePlugin(result: any) {
    ...
    const context = {
      ...
      set: (setter: (value: T) => void | PromiseLike<void>) =>
        this.#set(result, setter),
      root: !this.#parent,
    ...
    return executeAll.reduce((prevResult, plugin) => {
      return (
        safeCallback(() => plugin({ ...context, result: prevResult }))() ??
        prevResult
      );
    }, context.result);
  }
  ...
  #executeFinish(differResult = false, presetValue?: any) {
    ...
    if (this.#children?.length && !this._unsubscribeFlag) {
      this.#children
        .slice()
        .forEach((child) =>
          child._executeObserver(this._cacheRootPromise, presetValue, false)
        );
    }
  }
  ...
  #executeResult(
    ...
    const handlePromiseResult = (result: PromiseLike<any>) =>
      new Promise((resolve) =>
        result.then(
          (data) => {
            if (this._root?._rootPromise !== rootPromise || skipResult)
              return resolve(data);
            this.status = PromiseStatus.RESOLVED;
            // first time skip differ check
            if (this.#differ && status !== null)
              differResult =
                safeCallback(this.#differ)(this._v) ===
                safeCallback(this.#differ)(data);
            this._v = data;
            this.#afterSetValueCallbackList.forEach((fn) =>
              safeCallback(fn)(data)
            );
            resolve(data);
          },
          (error) => {
    ...
    if (result instanceof Promise || isPromiseLike(result)) {
        ...
        promise
          .finally(() => {
            try {
              ...
              if (
                finallyResult instanceof Promise ||
                isPromiseLike(finallyResult)
              ) {
      ...
      if (!skipResult) {
        ...
        if (this.#differ && status !== null)
          differResult =
            safeCallback(this.#differ)(this._v) ===
            safeCallback(this.#differ)(result);
        this._v = result;
        this.#afterSetValueCallbackList.forEach((fn) =>
          safeCallback(fn)(result)
        );
      }
  ...
  protected _executeObserver(
    ...
    if (
      !rootPromise ||
      this._root?._pauseFlag ||
      rootPromise !== this._root?._rootPromise
    )
      return;
    ...
    if (!this.#parent) {
      ...
      this.#executeNode(
        () => (active ? rootPromise : presetValue),
        rootPromise,
        status
      );
      // child node
      ...
      if (this.#reject) {
        this.#executeNode(
          () => this.#reject?.(rejectValuePromise),
          rootPromise,
          status
        );
